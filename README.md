...
The SO101 setup needs the Feetech SDK extra via `.[feetech]`. The dataset extra is included so `lerobot-record` works. The viz extra is required when using `--display_data=true`.
...
## Record Smoke Dataset

LeRobot's real-robot flow is teleoperate, record trajectories, then train a policy. Use the same teleoperator `id` for teleoperation, recording, and evaluation because LeRobot stores calibration files under that id.
